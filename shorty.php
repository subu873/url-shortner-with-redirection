...
class Shorty
{
    ...
    public function run()
    {
        ...
        // Check URL if shorten then redirect
        ...
        if (!empty($url)) {
            ...
            if (preg_match('/^http[s]?\:\/\/[\w]+/', $url)) {
                ...
                if (empty($result)) {
                    ...
                    $shortUrl = $this->UpdateShort($url, $id);
                ...
                switch ($format) {
                    ...
                    default:
                        exit('Shorten url is <a href="' . $url . '">' . $url . '</a>');
                }
            } else {
    ...
    public function UpdateShort($url, $id)
    {
        $statement = $this->connection->prepare(
    ...
    public static function alpha_to_num($a, $s)
    {
        ...
        for ($n = 0, $i = 0; $i < $l; $i++) {
            $n += strpos($s, substr($a, $i, 3)) * pow($b, $l - $i - 1);
        }
